...
import static com.example.Multi_reward.constant.Constants.DLT_SUFFIX;

/**
 * Kafka 消费失败兜底:原地重试 2 次(间隔 2s),仍失败则把消息连同异常信息
 * 发布到 <原topic>.DLT 死信队列(由 DeadLetterConsumer 记录),发奖消息不丢。
 * 反序列化失败这类重试不可能自愈的异常不重试,直接进死信。
...
public class KafkaErrorConfig {
    ...
    public DefaultErrorHandler kafkaErrorHandler(KafkaTemplate<Object, Object> kafkaTemplate) {
        ...
        errorHandler.addNotRetryableExceptions(KafkaMessageDeserializeException.class);
        return errorHandler;
    }
}
